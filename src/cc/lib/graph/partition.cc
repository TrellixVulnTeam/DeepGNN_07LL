...
Type Partition::GetNodeType(uint64_t internal_node_id, std::optional<Timestamp> node_ts, Timestamp &feature_flag) const
{
    auto type = m_node_types[internal_node_id];
    if (!node_ts.has_value() && type >= 0)
    {
        feature_flag = 0;
    }
